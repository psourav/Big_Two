...
public class BigTwoTable implements CardGameTable {
	...
	class BigTwoPanel extends JPanel implements MouseListener {
		...
		public void paintComponent(Graphics g) {		//Draw in BigTwo Panel
			...
			if (activePlayer == 0)				//Drawing PLAYER NAMES, AVATARS, LINES at their specified Pixel 
				g.setColor(Color.BLUE);			//Active Player = Blue Colored ; Non-Active Player = Black Color
			g.drawString("Player 0", 10, 20);
			g.setColor(Color.BLACK);
			g.drawImage(avatars[0], 10, 30, this);
		    g.drawLine(0, 130, 1500, 130);
			if (activePlayer == 1)
				g.setColor(Color.BLUE);
		    g.drawString("Player 1", 10, 150);
		    g.setColor(Color.BLACK);
		    g.drawImage(avatars[1], 10, 160, this);
		    g.drawLine(0,260, 1500, 260);
		    if (activePlayer == 2)
		    		g.setColor(Color.BLUE);
		    g.drawString("Player 2", 10, 280);
		    g.setColor(Color.BLACK);
		    g.drawImage(avatars[2], 10, 290, this);
		    g.drawLine(0, 390, 1500, 390);
		    if (activePlayer == 3)
		    		g.setColor(Color.BLUE);
		    g.drawString("Player 3", 10, 410);
		    g.setColor(Color.BLACK);
		    g.drawImage(avatars[3], 10, 420, this); 
		    g.drawLine(0, 520, 1500, 520);
		    if (activePlayer == 0) {				//Checking ActivePlayer and Displaying the Cards accordingly. //front image
		    ...
		    if (game.getHandsOnTable().size() == 0) {
		    		...
		    		g.setColor(Color.BLUE);
		    			...
		    			g.drawString("--------->Played by Player " + (((BigTwo) game).getLast()), 700, 600);
	...
	class RestartMenuItemListener implements ActionListener{
		public void actionPerformed(ActionEvent event) {
			...
			reset();
			game.getPlayerList().get(0).removeAllCards();	//removing cards from players
			game.getPlayerList().get(1).removeAllCards();
			game.getPlayerList().get(2).removeAllCards();
			game.getPlayerList().get(3).removeAllCards();
			game.getDeck().removeAllCards();
			game.getDeck().initialize();
			game.getDeck().shuffle();				//Shuffling it
			game.start((BigTwoDeck) game.getDeck() );	//starting again
